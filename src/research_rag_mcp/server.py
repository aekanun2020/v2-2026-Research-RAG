...
def input_errors(fn):
    ...
    async def checked(*args, **kwargs):
        try:
            ...
            if "workspace_id" in inspect.signature(fn).parameters:
                result = {**result, "workspace_id": kwargs.get("workspace_id", "default")}
            return result
...
def build_server(root):
    ...
    @server.tool()
    @input_errors
    def download_document(expected_revision: int, idempotency_key: str,
                          url: str | None = None, arxiv_id: str | None = None,
                          workspace_id: str = 'default') -> dict[str, Any]:
        """Download an authorized public HTTPS PDF URL OR arXiv ID on the server into the selected inbox. No client filesystem access is needed. This does NOT import, chunk or embed. Returns a durable job_id: poll job_status to completed and inspect its receipt (filename, URLs, SHA-256, bytes, pages). Then preview_inbox_document; call import_document separately only when authorized. Exactly one URL/ID; up to 50 MiB, 1000 pages, 5 redirects; no paywall bypass, OCR or synthesized replacement. Failed download is an error, never permission to save a project note instead. Exact retries reuse the key and all original arguments."""
        from .downloads import download_url
        download_url(url, arxiv_id)
        store, jobs = workspaces.context(workspace_id)
        return jobs.submit('download_document', dict(url=url, arxiv_id=arxiv_id,
            expected_revision=expected_revision, key=idempotency_key))

    @server.tool()
    @input_errors
    def document_ingestion_status(filename: str, workspace_id: str = 'default') -> dict[str, Any]:
        """Verify one inbox file by SHA-256 and report inbox-only versus imported, source identity/role/format, active chunks, workspace index status and a retained download receipt. Read-only. A downloaded PDF is not imported; an imported project note is not the original paper. Index counts do not certify extraction completeness or semantic quality."""
        store, jobs = workspaces.context(workspace_id)
        return store.document_ingestion_status(filename)

    @server.tool()
    @input_errors
    def start_project(topic: str, goal: str, unknowns: list[str], expected_revision: int, idempotency_key: str, workspace_id: str = 'default') -> dict[str, Any]:
        """Persist the researcher-supplied topic, goal and unanswered questions in the selected EMPTY workspace. For a separate project, create_workspace first and pass its workspace_id. Never clear or overwrite the old project, or invent a dataset, venue or scope."""
        store, jobs = workspaces.context(workspace_id)
    ...
    def import_document(filename: str, origin: str, role: Role, bibliography: Bibliography,
                        expected_revision: int, idempotency_key: str, document_id: str | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def search_literature(query: str, expected_revision: int, idempotency_key: str,
                          limit: int = 10, year_from: int | None = None, year_to: int | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def retrieve_evidence(query: str, roles: list[Role] | None = None, source_ids: list[str] | None = None,
                          limit: int = 8, mode: Literal["lexical", "semantic", "hybrid"] = "hybrid", workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def read_source_page(source_id: str, page_index: int, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def list_documents(offset: int = 0, limit: int = 50, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def list_chunks(source_id: str, chunk_set_id: str | None = None,
                    ...
                    offset: int = 0, limit: int = 50, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def read_chunk(chunk_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def get_chunk_context(chunk_id: str, before: int = 1, after: int = 1, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def inspect_document_chunks(source_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def rechunk_document(source_id: str, expected_revision: int, idempotency_key: str,
                         chunk_size_tokens: int = 512, chunk_overlap_tokens: int = 64, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def activate_chunk_set(chunk_set_id: str, expected_revision: int, idempotency_key: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def set_chunk_status(chunk_id: str, status: Literal['active','excluded','needs_review'], reason: str,
                         expected_revision: int, idempotency_key: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def find_evidence_usage(chunk_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def search_index_status(workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def rebuild_search_index(expected_revision: int, idempotency_key: str,
                             source_ids: list[str] | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def explore_topics(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def map_research_gaps(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def formulate_question(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def design_study(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def track_execution(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def interpret_results(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def summarize_dataset(source_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def draft_manuscript(query: str, limit: int = 8, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def prepare_submission(manuscript_id: str, guideline_source_id: str, submission_id: str | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def save_artifact(stage: Stage, title: str, blocks: list[Block], limitations: list[str], dependency_ids: list[str],
                      expected_revision: int, idempotency_key: str, artifact_id: str | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def read_artifact(artifact_id: str, version: int | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def export_manuscript(manuscript_id: str, expected_revision: int, mode: Literal['draft', 'reviewed'] = 'draft',
                          submission_id: str | None = None, guideline_source_id: str | None = None, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def backup_workspace(workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def preview_workspace_cleanup(scope: Literal['search_index', 'documents', 'workspace', 'all_except_inbox'], workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def job_status(job_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def resume_job(job_id: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def migrate_legacy_workspace(expected_revision: int, idempotency_key: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def list_sources(offset: int = 0, limit: int = 50, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def add_context(content: str, title: str, expected_revision: int, idempotency_key: str,
                    ...
                    format: Literal['md','txt','csv','json'] = 'md', workspace_id: str = 'default') -> dict[str, Any]:
        """Import explicitly requested client-supplied text via a durable job. Default role is project_note, not observed findings or the original cited paper. Never use this as a substitute when original PDF download/import was requested or failed. For a public PDF use download_document, preview, then authorized import_document. Requires the actual supplied title/source; preserves that text and its provenance. Poll job_status with this workspace_id."""
        store, jobs = workspaces.context(workspace_id)
    ...
    def add_directory(path: str, bibliography_by_filename: dict[str, Bibliography], expected_revision: int,
                      idempotency_key: str, role: Role = 'literature', workspace_id: str = 'default') -> dict[str, Any]:
    ...
    @server.tool()
    @input_errors
    def restore_workspace(snapshot: str, expected_revision: int, idempotency_key: str, workspace_id: str = 'default') -> dict[str, Any]:
    ...
    def list_workspaces() -> dict[str, Any]:
        """List independent workspaces and registry_revision. No workspace is switched globally. Include workspace_id in every subsequent call. IDs do not provide access control."""
        return workspaces.list()

    @server.tool()
    @input_errors
    def create_workspace(name: str, expected_registry_revision: int, idempotency_key: str) -> dict[str, Any]:
        """Create an empty independent workspace with its own files, journal, Qdrant collection and jobs. Read list_workspaces first for registry_revision. Exact retries return the same ID. Does not clear, migrate, copy or import anything from an existing workspace; does not start a research project."""
        return workspaces.create(name, expected_registry_revision, idempotency_key)
