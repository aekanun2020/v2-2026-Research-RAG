...
class QdrantService(StorageService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not collection_exists:
                ...
                # Check vector size
                self.logger.debug(f"Collection '{self.collection_name}' exists, checking vector size")
    ...
    async def search(
        ...
        try:
            ...
            # Handle QueryResponse object - query_points returns QueryResponse with .points attribute
    ...
    async def list_sources(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"Scrolling Qdrant collection '{self.collection_name}' to list sources")
            ...
            # Handle both old and new Qdrant client versions
            if isinstance(scroll_results, tuple) and len(scroll_results) > 0:
                # New format: (points, next_page_offset)
                points = scroll_results[0]
            elif hasattr(scroll_results, 'points'):
                # Old format: object with .points attribute
