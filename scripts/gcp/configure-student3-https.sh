#!/bin/bash
set -euo pipefail
[[ "$(hostname)" == student3-research-rag ]]
...
backup_dir=/srv/research-rag/deploy/backup-student3-20260924
test ! -e "$backup_dir"
...
for name in ['/srv/research-rag/edge/Caddyfile','/srv/research-rag/deploy/runtime.env']:
 ...
 p.write_text(s.replace('34-142-222-110.sslip.io','35-240-233-160.sslip.io'))
PY
...
echo 'Student3 hostname correction applied; external TLS/MCP verification required.'
